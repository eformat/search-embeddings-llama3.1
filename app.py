...
def get_embedding(text):
    ...
    response = requests.post('http://localhost:8080/v1/embeddings', headers=headers, data=data)
    
    if response.status_code != 200:
        raise Exception(f"API request failed with status code {response.status_code}: {response.text}")
    
    response_data = response.json()
    #print(f"API Response: {json.dumps(response_data['data'][0]['embedding'][0], indent=2)}")

    if 'embedding' in response_data['data'][0]:
        return np.array(response_data['data'][0]['embedding'][0], dtype=np.float32)
