...
def get_embedding(text):
    ...
    response = requests.post('https://nomic-embed-text-v1-5-maas-apicast-production.apps.prod.rhoai.rh-aiservices-bu.com/embed', headers=headers, data=data)
...
def build_annoy_index(conn, vector_size=VECTOR_SIZE, n_trees=10):
    c = conn.cursor()
...
def add_question(conn, text):
    ...
    build_annoy_index(conn, vector_size=VECTOR_SIZE)
